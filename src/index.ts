...
function extractContent(customFields: any): string {
  if (!customFields) return "";
  const udf = customFields.udf2rtxt;
  if (!udf) return "";
  if (typeof udf === "string") return udf;
  if (udf.content) return udf.content;
...
const TOOLS = [
  {
    ...
    description:
      "List all IT Documentation documents in SuperOps. Optionally filter by client name. " +
    ...
    inputSchema: {
      ...
      properties: {
        client: {
          type: "string",
          description:
            "Optional: filter to a specific client name (partial match, case-insensitive). " +
  ...
  },
  {
    ...
    description:
      "Get the full content of a specific IT Documentation document. " +
    ...
    inputSchema: {
      ...
      properties: {
        name: {
          type: "string",
          description:
            "Document name to search for (partial match, case-insensitive). " +
        ...
        client: {
          type: "string",
          description:
            "Optional: narrow search to a specific client name (partial match).",
        },
        plainText: {
          type: "boolean",
          description:
            "If true, return plain text instead of HTML (default: true for readability).",
        },
      },
    },
  },
  {
    ...
    description:
      "Search IT Documentation by keyword. Searches across both document names and " +
    ...
    inputSchema: {
      ...
      properties: {
        query: {
          type: "string",
          description:
            "Search query (searches doc names and content, case-insensitive). " +
  ...
  },
  {
    ...
    description:
      "Update the content of an existing IT Documentation document in SuperOps. " +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        name: {
          type: "string",
          description:
            "Document name to find (partial match). Use if docId is not known.",
        },
        client: {
          type: "string",
        ...
        content: {
          type: "string",
          description:
            "The new HTML content for the document. Can include tags like <p>, <table>, " +
        ...
        append: {
          type: "boolean",
          description:
            "If true, append the content to the existing document instead of replacing. " +
...
async function handleListDocs(args: any) {
  ...
  if (args.client) {
    ...
    docs = docs.filter((d: any) =>
      getClientName(d.client).toLowerCase().includes(filter)
    );
  }
...
async function handleGetDoc(args: any) {
...
async function handleSearchDocs(args: any) {
  ...
  const nameMatches = docs.filter((d: any) =>
  ...
  const contentCandidates = docs.filter(
    (d: any) => !d.name.toLowerCase().includes(query)
  );
...
const server = new Server(
  { name: "superops-docs-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
